...
export interface StoreAdapter {
  ...
  update<A>(updater: (transaction: StoreTransaction) => A): A
}
...
export function bind(schema: Schema, store: StoreAdapter, ref: Ref): Value {
  if (schema.kind === "string") {
    return {
      ...
      set: (value: string) => {
        store.update((transaction) => transaction.setString(ref, value))
      },
    }
  }
    ...
    return {
      ...
      insert(index: number, value: unknown) {
        store.update((transaction) => {
          const child = createInTransaction(arraySchema.element, transaction, value)
          transaction.insertArrayItem(ref, index, child)
        })
      },
      remove(index: number) {
        store.update((transaction) => transaction.removeArrayItem(ref, index))
      },
...
export function create(schema: Schema, store: StoreAdapter, json: unknown): Ref {
  return store.update((transaction) => createInTransaction(schema, transaction, json))
}

function createInTransaction(schema: Schema, transaction: StoreTransaction, json: unknown): Ref {
  if (schema.kind === "string") {
    if (typeof json !== "string") {
      throw new TypeError("Expected string value")
    }

    return transaction.createString(json)
  }
  ...
  if (schema.kind === "array") {
    ...
    const children = json.map((value) =>
      createInTransaction(arraySchema.element, transaction, value),
    )

    return transaction.createArray(children)
  }
...
export class FlatStore implements StoreAdapter {
  ...
  private currentTransaction: StoreTransaction | null = null
  ...
  update<A>(updater: (transaction: StoreTransaction) => A): A {
    if (this.currentTransaction) return updater(this.currentTransaction)
  ...
  private createTransaction(): StoreTransaction {
    return {
      ...
      insertArrayItem: (ref, index, child) => {
        const items = this.array(ref).items
        checkInsertIndex(index, items.length)
        this.node(child)
        items.splice(index, 0, child)
      },
      removeArrayItem: (ref, index) => {
        const items = this.array(ref).items
        checkIndex(index, items.length)
...
export class YjsStore implements StoreAdapter {
  ...
  private createTransaction(): StoreTransaction {
    return {
      ...
      insertArrayItem: (ref, index, child) => {
        const node = this.arrayNode(ref)
        checkInsertIndex(index, node.length)
        this.node(child)
...
function checkInsertIndex(index: number, length: number): void {
  if (!Number.isInteger(index) || index < 0 || index > length) {
    throw new RangeError("Array index out of bounds")
  }
}
